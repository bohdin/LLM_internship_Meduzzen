...
from utils import get_gpt_response, get_transcription, transcripts_and_summaries_log
...
def main() -> None:
    ...
    while True:
        ...
        for audio_path in audio_paths:
            ...
            logger.info(f"-> Transcript: {transcription_input}")

            if mode == "custom":
                while True:
                    new_prompt = input(
                        "Enter prompt for GPT (write 'default' for return default prompt): "
                    )

                    instructions = (
                        dict_gpt_task["summary"]
                        if new_prompt == "default"
                        else new_prompt
                    )

                    summary = get_gpt_response(
                        client, model_name, instructions, transcription_input
                    )

                    print(f"\n-> GPT Summary: {summary}", end="\n\n")

                    transcripts_and_summaries_log(
                        mode, instructions, transcription_input, summary
                    )
